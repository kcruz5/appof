...
GROUP BY ime.idinBodega, ime.fechaIngreso) as x 
Where x.idinCte = case when '$cliente'=0 then x.idinCte else '$cliente' end
ORDER BY x.fechaIngreso desc";
    ...
    if(mysqli_num_rows($result) > 0)
    {
        ...
        <td></td><td></td><td></td><td></td><td></td><td>Q. '.number_format($costo,2).'</td><td>Q. '.number_format($venta,2).'</td><td>Q. '.number_format($utilidad,2).'</td></tr>
        </tfoot>';
    }
    else
    {
