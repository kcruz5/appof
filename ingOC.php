...
(select count(*) from in_ajuste_enc where estado=1) as notificacion";
    ...
    function tipOC(){
        include("action/keyaction.php");
        $str= "select idinTipoOC, descripcion from in_tipo_oc where estado=1";
		$resultset = mysqli_query($con, $str) or die("Error de base de datos:". mysqli_error($con));
		while (	$row = mysqli_fetch_array($resultset, MYSQLI_ASSOC)) {
            echo "<option value='".$row["idinTipoOC"]."'>".$row["descripcion"]."</option>";
	    } 
              mysqli_close($con);
    }   
    function cliente(){
  ...
  <div class="row">
    <div class="col-sm-3">
      <div class="pull-right">
         ...
         <br>    <div class="col-sm-3">    <div class="pull-right">
          <div class="input-group">							 
				<select class="form-control"  id="tipOC" name="tipOC" title="Tipo OC">
                        <?php $db = new clsMenu(); $db->tipOC();?>
                </select>
		</div>
      </div>
    </div>
      ...
      <div class="col-sm-2">
      <div class="pull-right">
        <button id="nuevo" name="nuevo" class="btn btn-primary" title="Crear Orden de Compra" onclick="readRecords()"><i class="fas fa-sign-in-alt"></i></button>
          <button id="crea" name="crea" class="btn btn-info" title="Guardar Orden de Compra" onclick="saveRecords()"><i class="fas fa-save"></i></button>
           <button id="confirm" name="confirm" class="btn btn-success" title="Confirmar Orden de Compra" onclick="ConfirmRecords()"><i class="fas fa-clipboard-check"></i></button>
          <button id="abono" name="abono" class="btn btn-secondary" title="Abono a Orden de Compra" onClick="Abono()"><i class="fas fa-hand-holding-usd"></i></button>
          <button id="exit" name="exit" class="btn btn-danger" title="Salir" onclick="exit()"><i class="fas fa-door-open"></i></button>
      </div>
    </div>
  </div>
  <br>
...
<table id="data"  class="table table-striped table-responsive">
              ...
              <div class="row"> <div class="col-md-12">
                  <div class="col-md-4" id="descuento">                                     <div class="form-inline">
<label for="descto">Descuento:</label><input type="text" placeholder="0.00" class="form-control" id="descto" name="descto"/>
                  </div>  </div>
                <div class="col-md-8">
    ...
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
      <script src="class/ingOC.js"></script>
  </body>
