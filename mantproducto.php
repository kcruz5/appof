<?php
    ...
    function proveedor(){
        include("action/keyaction.php");
        $str= "select idinProveedor,nombre from in_proveedor where estado=1";
		$resultset = mysqli_query($con, $str) or die("Error de base de datos:". mysqli_error($con));
		while (	$row = mysqli_fetch_array($resultset, MYSQLI_ASSOC)) {
            echo "<option value='".$row["idinProveedor"]."'>".$row["nombre"]."</option>";
	    }
            mysqli_close($con);
    }
    function categoria(){
        include("action/keyaction.php");
        $str= "select idinProdCat, nombre from in_prod_categoria where estado=1";
		$resultset = mysqli_query($con, $str) or die("Error de base de datos:". mysqli_error($con));
		while (	$row = mysqli_fetch_array($resultset, MYSQLI_ASSOC)) {
            echo "<option value='".$row["idinProdCat"]."'>".$row["nombre"]."</option>";
	    } 
            mysqli_close($con);
    }
    function bodega(){
...
<style type="text/css">
    ...
    table.table td a.price {color: #39AD1B;}
    table.table td a.um{color: #172B96;}    
    table.table td a.bodega{color: #7C7C7C;}    
</style>
  ...
  <body id="page-top" onload="nobackbutton();">
    <nav class="navbar navbar-expand navbar-dark bg-dark static-top">
      ...
      <ul class="navbar-nav ml-auto ml-md-0">
        <?php $db=new clsMenu(); $db->notificacion(); ?>
...
<?php            $db = new clsMenu();          $db->menubar(); ?>
        ...
        <div class="container-fluid">
          <ol class="breadcrumb">
            <li class="breadcrumb-item">
              <a href="main.php">Mantenimiento Productos</a>
            </li>
          </ol>
      ...
      <div class="pull-right">
          <div class="search-box">
            <div class="input-group">								
				<input type="text" id="search" class="form-control" placeholder="Buscar por descripción">
  ...
  <div class="row">
                ...
                <div class="form-group">                      
                    <label for="proveedor" class="control-label">Proveedor:</label>
                      <select class="form-control" id="proveedor" name="proveedor">
                        <?php $db = new clsMenu(); $db->proveedor();?>
                      </select>
                  </div>  
                <div class="form-group">                      
                    <label for="categoria" class="control-label">Categoría:</label>
                      <select class="form-control" id="categoria" name="categoria">
                        <?php $db = new clsMenu(); $db->categoria();?>
                      </select>
                  </div>  
                <div class="form-group">
                    <label for="um" class="control-label">Medida:</label>
                    <input type="text" value="" class="form-control" id="um" name="um"/>
                  </div>
                <div class="form-group">
                    <label for="precompra" class="control-label">Precio Compra:</label>
                    <input type="text" value="" class="form-control" id="precompra" name="precompra"/>
                  </div>
                <div class="form-group">
                    <label for="preventa" class="control-label">Precio Venta:</label>
                    <input type="text" value="" class="form-control" id="preventa" name="preventa"/>
                  </div>
                 <div class="form-group">
                    <label for="presp1" class="control-label">Precio Especial 1:</label>
                    <input type="text" value="" class="form-control" id="presp1" name="presp1"/>
                  </div>
                <div class="form-group">
                    <label for="presp2" class="control-label">Precio Especial 2:</label>
                    <input type="text" value="" class="form-control" id="presp2" name="presp2"/>
                  </div>
                <div class="form-group">
                    <label for="presp3" class="control-label">Precio Especial 3:</label>
                    <input type="text" value="" class="form-control" id="presp3" name="presp3"/>
                  </div>                
            </div>
            <div class="modal-footer">
    ...
    <div class="modal" id="update_modal" tabindex="-1" role="dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                 <div class="form-group">
                    <label for="nombre" class="control-label">Nombre:</label>
                    <input type="text" value="" class="form-control" id="update_nombre" name="update_nombre"/>
                  </div>
                <div class="form-group">                      
                    <label for="proveedor" class="control-label">Proveedor:</label>
                      <select class="form-control" id="update_proveedor" name="update_proveedor">
                        <?php $db = new clsMenu(); $db->proveedor();?>
                      </select>
                  </div>  
                <div class="form-group">                      
                    <label for="categoria" class="control-label">Categoría:</label>
                      <select class="form-control" id="update_categoria" name="update_categoria">
                        <?php $db = new clsMenu(); $db->categoria();?>
                      </select>
                  </div>                  
            </div>
    ...
    <div class="modal" id="precio_modal" tabindex="-1" role="dialog">
		  <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">                
            ...
            <div class="modal-body">
                 <div class="form-group">
                    <label for="precompra" class="control-label">Precio Compra:</label>
                    <input type="text" value="" class="form-control" id="update_precompra" name="update_precompra"/>
                  </div>
                <div class="form-group">                      
                    <label for="preventa" class="control-label">Precio Venta:</label>
                     <input type="text" value="" class="form-control" id="update_preventa"/>
                  </div>  
                <div class="form-group">                      
                    <label for="presp1" class="control-label">Precio Especial 1:</label>
                     <input type="text" value="" class="form-control" id="update_presp1"/>
                  </div>                   
                <div class="form-group">                      
                    <label for="presp2" class="control-label">Precio Especial 2:</label>
                     <input type="text" value="" class="form-control" id="update_presp2"/>
                  </div>   
                <div class="form-group">                      
                    <label for="presp3" class="control-label">Precio Especial 3:</label>
                     <input type="text" value="" class="form-control" id="update_presp3"/>
                  </div>                   
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                <button type="button" class="btn btn-primary" onclick="validaPrecio()">Actualizar</button>
                <input type="hidden" id="hidden_id">
            </div>
        </div>
    </div>
	</div>  
    <div class="modal" id="um_modal" tabindex="-1" role="dialog">
		  <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">                
            ...
            <div class="modal-body">
                 <div class="form-group">
                    ...
                    <input type="text" value="" class="form-control" id="update_um" name="update_um"/>
                  </div>            
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                <button type="button" class="btn btn-primary" onclick="validaUM()">Actualizar</button>
                <input type="hidden" id="hidden_id">
            </div>
        </div>
    </div>
	</div>  
    <div class="modal" id="bodega_modal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">                
                <h4 class="modal-title">Asignar Producto a Bodega</h4>
                <button class="close" type="button" data-dismiss="modal" aria-label="Cerrar">
              <span aria-hidden="true">×</span>
            </button>
            </div>
            <div class="modal-body">
				<div class="input-group" >							
					<select class="form-control" id="update_bodega" name="update_bodega">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                <button type="button" class="btn btn-primary" onclick="UpdateBodega()"><i class="fas fa-check-square"></i> Asignar</button>
                <input type="hidden" id="hidden_id">
            </div>
        </div>
    </div>
...
<script src="//cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/toastr.min.js"></script>
      <script src="class/mantproducto.js"></script>
  </body>
