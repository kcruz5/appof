<?php
    ...
    function tipo(){
        include("action/keyaction.php");
        $str= "select idinTipoAjuste,descripcion from in_tipo_ajuste where estado=1";
		$resultset = mysqli_query($con, $str) or die("Error de base de datos:". mysqli_error($con));
		while (	$row = mysqli_fetch_array($resultset, MYSQLI_ASSOC)) {
            echo "<option value='".$row["idinTipoAjuste"]."'>".$row["descripcion"]."</option>";
	    } 
    }         
...
?>
  ...
  <div class="row">
    <div class="col-sm-4">
      ...
      <div class="pull-right">
          <div class="input-group">							 
								<select class="form-control"  id="tipoajuste" name="tipoajuste" title="Tipo de Ajuste">
                                <?php $db = new clsMenu(); $db->tipo();?>
                                </select>
							</div>
      </div>
    </div>
      ...
      <div class="col-sm-4">
      <div class="pull-right">
        <button id="nuevo" name="nuevo" class="btn btn-primary" title="Crear Ajuste de Inventario" onclick="readRecords()"><i class="fas fa-sign-in-alt"></i></button>
          <button id="crea" name="crea" class="btn btn-info" title="Guardar Ajuste de Inventario" onclick="saveRecords()"><i class="fas fa-save"></i></button>
          <button id="confirm" name="confirm" class="btn btn-success" title="Confirmar Ajuste de Inventario" onclick="ConfirmRecords()"><i class="fas fa-clipboard-check"></i></button>
      </div>
    </div>
  </div>
  <br>
...
<table id="data"  class="table table-striped table-responsive">
    ...
    <script src="class/inAjuste.js"></script>
  </body>
